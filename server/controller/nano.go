...
func (nc NanoController) Callback(confirmationResponse net.ConfirmationResponse) {
	...
	if block["link_as_account"] == nc.DonationAccount && block["link_as_account"] != block["account"] {
		...
		if doReRandom {
			...
			// Lock refund
			lock, err := db.GetDB().Locker.Obtain(db.CTX, fmt.Sprintf("pawnimal:refundl:%s", hash), 100*time.Second, nil)
			...
			lock, err := db.GetDB().Locker.Obtain(db.CTX, fmt.Sprintf("pawnimal:callback_lock:%s", hash), 100*time.Second, nil)
			if err == redislock.ErrNotObtained {
				return
			} else if err != nil {
				glog.Error(err)
